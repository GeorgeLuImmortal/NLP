...
def get_euclidean(text1, text2):
     ...
     sum1 = sum([(vec1[x] - vec2[x])**2 for x in combination]) #count euclidean distance
     
     result = math.sqrt(sum1)                                   #count euclidean distance
     if not result:
        return 0.0
     else:
...
#text1 = 'q'
#text2 = 'd4'
#euclidean = get_euclidean(text1, text2)
...
for i in range(0,p):                                      
    print(get_euclidean(array[i][0],array[i][1]),array[i][0],array[i][1])  #print all cosine similarity of combinations
